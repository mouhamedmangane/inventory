...
class Nouveau extends Component
{
    ...
    {
        if(!$clients)
            $this->clients=Client::all()->pluck('nom','nom');
        else
    ...
    public function getColumns(){
        ...
        $columns[]= GCellFactory::text('Quantité Reçue','quantite','quantite')
                    ->type('numeric')
                    ->defaultValue('0');
        $columns[]= GCellFactory::text('Quantité Demandée','quantite','quantite')
                    ->type('numeric')
                    ->defaultValue('0');
