        ...
        this.dataRestant= async function(i){
            ...
            if(this.unique==true){
                let valeurADessines=toutesLesValeurs.filter((o)=> { 
                    return $.inArray(o.value,data)==-1 || o.value==data[i];
                 });
    ...
    $.fn.NplEditorTable.GCellSelectFree=function(ob){
        ...
        this.optionsSelectData = async function(i){
            ...
            if(!tab && valueDep){
                $.chargement();
                response=null;
                await this.getOptionAjax(valueDep).fail(
                    function(){$.rmchargement();}
                );
                if(reponse){
                    v   console.log(response.data);
        ...
        this.getOptionAjax=(valueDep)=>{
            console.log(this.urlDep+'/'+valueDep,)
            return $.ajax({
                type: 'get',
